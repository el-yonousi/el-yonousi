...
export default defineEventHandler(async (event) => {
    ...
    const { data } = await client.from("testimonials").select("id, profile, fullName, recommendation, relationship, status, linkedin").eq('display', true).order('order', { ascending: true });
    return data;
});
